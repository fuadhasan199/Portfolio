...
const Projects = () => {
  ...
  const projects = [
    ...
    },
    {
      ...
      futurePlans: "Adding a user review system and a search/filter feature for easier app discovery."
    },
    {
    ...
    <section id="projects" className="py-24 bg-gray-50">
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {projects.map((project, index) => (
            <div key={index} className="bg-white rounded-2xl overflow-hidden shadow-md hover:shadow-xl transition duration-300 group flex flex-col">
      ...
      {selectedProject && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4 backdrop-blur-sm">
          <div className="bg-white w-full max-w-3xl max-h-[90vh] overflow-y-auto rounded-3xl relative p-8 shadow-2xl">
            {/* Close Button */}
            <button 
              onClick={() => setSelectedProject(null)} 
              className="absolute top-4 right-4 p-2 bg-gray-100 rounded-full hover:bg-red-100 text-gray-600 hover:text-red-600 transition-colors"
            >
              <X size={24} />
            </button>

            <h3 className="text-2xl md:text-3xl font-bold text-gray-900 mb-6">{selectedProject.name}</h3>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {Array.isArray(selectedProject.tech) ? selectedProject.tech.map(t => (
                    <span key={t} className="px-3 py-1 bg-purple-50 text-purple-700 rounded-lg text-xs font-bold border border-purple-100">{t}</span>
                  )) : <span className="text-sm">{selectedProject.tech}</span>}
                </div>
              </div>

              {/* Description */}
              <p className="text-gray-600 leading-relaxed text-sm md:text-base">{selectedProject.description}</p>
